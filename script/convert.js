...
function convertFile(f) {
    ...
    if (f.type == "image/svg+xml") {
        confirmFilesize(f.size, 1024*1024*1.5, function(f) {
            ...
            var r = new FileReader()
            r.addEventListener("load", function(e) {
                setTimeout(compress.svg, 200, e.target.result, f.name || "unnamed.svg");
            })
...
function convertString(s) {
    ...
    if (s.match(/^(?:[\s\n]*<[?!][^>]+>)*[\s\n]*<svg(?:>|[\s\n])(?:.|\n)*<\/svg\s*>\s*$/i)) {
        confirmFilesize(s.length, 1024*1024*1.5, function(s) {
            document.body.classList.add("loading");
            window.type = "svg";
            setTimeout(compress.svg, 100, s, "clipboard.svg");
        }.bind(this, s));
    } else {
        if (s.match(/(?:[\s\n]*<[?!][^>]+>)*[\s\n]*<!doctype\s+html/i)) { window.mime = "text/html"; window.ext = "html" }
        else if (s.match(/(?:[\s\n]*<[?!][^>]+>)*[\s\n]*<!doctype\s/i) || s.match(/<\?xml/i)) { window.mime = "application/xml"; window.ext = "xml" }
        else { window.mime = ""; window.ext = "txt" }
        confirmFilesize(s.length, 1024*1024*10, function(s) {
            window.type = "plain";
            result.set(null, getUri(s, window.mime || "text/plain"), { original: s.length, compressed: null });
            updateDownloadLink("clipboard." + (window.ext || "txt"));
            document.body.classList.remove("loading");
        }.bind(this, s));
    }
}
